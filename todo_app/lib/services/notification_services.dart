// this is the code i copied from lesson 196 (Scheduled Notification)
...
class NotifyHelper {
  ...
  cancelAllNotification() async {
    ...
    print("All Notifications is Cancelled!");
  }
  ...
  tz.TZDateTime _nextInstanceOfTenAM(
    ...
    if (scheduledDate.isBefore(now)) {
      if (repeat == "Daily") {
        //scheduledDate = scheduledDate.add(const Duration(days: 1));  // this way is true for days but we will use another way
        scheduledDate = tz.TZDateTime(tz.local, now.year, now.month,
            (formattedDate.day) + 1, hour, minutes);
      }
      if (repeat == "Weekly") {
        // both of the next ways is not true...
...
// the code I wrote myself ... in lesson 192 (Local Notification)
...
class NotifyHelper {
  ...
  // this is just for let my device to get ready to receive notifications
  initializeNotification()async{
    tz.initializeTimeZones();
    //tz.setLocalLocation(tz.getLocation(timeZoneName));
    ...
    final IOSInitializationSettings initializationSettingsIOS = IOSInitializationSettings(
      ...
      requestBadgePermission: false,
      requestAlertPermission: false,
      onDidReceiveLocalNotification: onDidReceiveLocalNotification,  // for older versions
    );
    /*
    // we are not working on macOS
  ...
  displayNotification({required String title , required String body,}) async{
    ...
    const AndroidNotificationDetails(
      'your channel id',
      'your channel name',
      'your channel description',  // channelDescription : '', .. was making an error and i deleted it
  ...
  // it is just for older ios versions
  Future onDidReceiveLocalNotification(
